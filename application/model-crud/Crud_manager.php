...
class Crud_manager extends Crud_model {
    ...
    public function file_validated_handle($validate, $form_data) {
        foreach ($validate as $field => $item) {
            $form = $this->schema[$field]['form'];
            if (isset($form['type']) && ($form['type'] == "file" || $form['type'] == "multiple_file")
            ) {
    ...
    public function is_in_group($group_name, $break = FALSE) {
