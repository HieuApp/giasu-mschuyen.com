    ...
    protected function _set_where($params) {
        if (count($params) == 1 && is_array($params[0])) {
            foreach ($params[0] as $field => $filter) {
                if (is_array($filter)) {
                    $this->_database->where_in($this->_table_alias . "." . $field, $filter);
                } else {
                    if (is_int($field)) {
                        $this->_database->where($filter);
                    } else {
                        $this->_database->where($this->_table_alias . "." . $field, $filter);
                    }
                }
            }
            ...
            if (is_array($params[1])) {
                $this->_database->where_in($this->_table_alias . "." . $params[0], $params[1]);
            } else {
                $this->_database->where($this->_table_alias . "." . $params[0], $params[1]);
            }
        } else if (count($params) == 3) {
            $this->_database->where($this->_table_alias . "." . $params[0], $params[1], $params[2]);
        } else {
            if (is_array($params[1])) {
                $this->_database->where_in($this->_table_alias . "." . $params[0], $params[1]);
            } else {
